...
dq_2A_dp_1A <- (new_out1A[7] - orig_out[7]) / epsilon #dq2A/dp1A
dq_2B_dp_1A <- (new_out1A[8] - orig_out[8]) / epsilon #dq2B/dp1A
...
dq_2A_dp_1B <- (new_out1B[7] - orig_out[7]) / epsilon #dq2A/dp1B
dq_2B_dp_1B <- (new_out1B[8] - orig_out[8]) / epsilon #dq2B/dp1B
...
dq_1A_dp_2A <- (new_out2A[5] - orig_out[5]) / epsilon #dq1A/dp2A
dq_1B_dp_2A <- (new_out2A[6] - orig_out[6]) / epsilon #dq1B/dp2A
...
dq_1A_dp_2B <- (new_out2B[5] - orig_out[5]) / epsilon #dq1A/dp2B
dq_1B_dp_2B <- (new_out2B[6] - orig_out[6]) / epsilon #dq1B/dp2B
dq_2A_dp_2B <- (new_out2B[7] - orig_out[7]) / epsilon #dq2A/dp2B
dq_2B_dp_2B <- (new_out2B[8] - orig_out[8]) / epsilon #dq2B/dp2B

# Calculate price differences
p_1A_est = (
  eq_int_good$w_1A * dq_1A_dp_1A - (eq_downstream_p$p_1B - eq_int_good$w_1B) *
    dq_1B_dp_1A - eq_int_good$x_1A
) / dq_1A_dp_1A

p_1B_est = (
  eq_int_good$w_1B * dq_1B_dp_1B - (eq_downstream_p$p_1A - eq_int_good$w_1A) *
    dq_1A_dp_1B - eq_int_good$x_1B
) / dq_1B_dp_1B

p_2A_est = (
  eq_int_good$w_2A * dq_2A_dp_2A - (eq_downstream_p$p_2B - eq_int_good$w_2B) *
    dq_2B_dp_2A - eq_int_good$x_2A
) / dq_2A_dp_2A

p_2B_est = (
  eq_int_good$w_2B * dq_2B_dp_2B - (eq_downstream_p$p_2A - eq_int_good$w_2A) *
    dq_2A_dp_2B - eq_int_good$x_2B
) / dq_2B_dp_2B

price_err <- (
  sqrt(abs(p_1A_est ^ 2 - eq_downstream_p$p_1A ^ 2)) 
  + sqrt(abs(p_1B_est ^ 2 - eq_downstream_p$p_1B ^ 2))
  + sqrt(abs(p_2A_est ^ 2 - eq_downstream_p$p_2A ^ 2))
  + sqrt(abs(p_2B_est ^ 2 - eq_downstream_p$p_2B ^ 2))
)
