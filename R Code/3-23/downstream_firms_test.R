...
d_firm_main <- function(p_1, p_2, w_A, w_B, M, firm_1){
  ...
  # quantity demanded of intermediate goods# Linear production
  ...
  d_firm_1 <- function(w, p, odds, M) {
    ...
    pi <- x_1A * (p_A - w_A - down_mc_1 - m_ineff) + x_1B * (p_B - w_B - down_mc_1)  # assume consumers buy one of each good
  ...
  d_firm_2 <- function(w, p, odds, M) {
    ...
    pi <- x_2A * (p_A - w_A - down_mc_2) + x_2B * (p_B - w_B - down_mc_2)  # assume consumers buy one of each good
