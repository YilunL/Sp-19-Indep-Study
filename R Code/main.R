...
m_ineff <-
  0 * integrated # increase in MC due to integration (unimplemented)
foreclosure <-
  0 * integrated # dummy to see if firms 1-A stops selling to firm 2 (unimplemented)


M = 1  # market size

set.seed(103) #set a random seed
...
# create a list of intermediate good characteristics, including wholesale prices and quantities
eq_int_good <-
  list(
...
if (integrated == 0) {
  for (run in 1:nruns) {
    ...
    # tolerance limits to exit the optimization
    if (run == 1) {
      out_tol_limit = 1E-10
    } else if (run == 2) {
      out_tol_limit = 1E-9
    ...
    # tracker for number of iterations the downstream firms have gone through
    downstream_iter = 0
    
    # loop for upstream firm optimization
    while ((out_tol > out_tol_limit) & (downstream_iter < 100000)) {
      optim_A = sample(c(0, 1), 1)  # randomize which firm gets to make first offer
      ...
      # optimize firm A given firm B wholesale prices are fixed
      if (optim_A == 1) {
        firm_A_optim <-
          optim(
        ...
        # return optimal wholesale prices
        ...
        # optimize firm B given firm A wholesale prices are fixed
        firm_B_optim <-
          optim(
...
if (downstream_iter >= 100000) {
  cat(
    "Warning: Upstream firms did not converge within relative tolerance. Check FOCs to ensure that you have indeed found an equilibrium"
  )
}
